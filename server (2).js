  ...
  server.tool(
    ...
    async ({ model, input }) => {
      try {
        ...
        // Safety net: Kling 3.0 requires duration + mode; fill sensible defaults if missing.
        if (model.startsWith("kling-3.0")) {
          if (finalInput.duration === undefined) finalInput.duration = "5";
          if (finalInput.mode === undefined) finalInput.mode = "pro";
          if (!finalInput.image_urls && !finalInput.aspect_ratio) finalInput.aspect_ratio = "9:16";
        }
        const taskId = await kieCreate(model, finalInput);
...
app.get("/", (_req, res) => res.send("kie.ai MCP server is running — BUILD: kling3-v2.1. Add /<your-secret>/mcp to Claude as a custom connector."));
